        ...
        return recentProjects.map(project => ({
            ...
            click: async () => {
                try {
                    ...
                    await dialog.showErrorBox('LEAR Error', `Failed to open project: ${error.message}`);
...
async function createNewProject(parentWin) {
    try {
        const { canceled, filePath } = await dialog.showSaveDialog(parentWin, {
            ...
            buttonLabel: 'Create',
            filters: [{ name: 'LEAR Project', extensions: [''] }],
        ...
        await dialog.showErrorBox('LEAR Error', `Failed to create project: ${error.message}`);
        return null;
    }
}
...
async function openProject(parentWin) {
    try {
        ...
        // Check if this is a valid LEAR project
        await fs.access(dataDir);
        await fs.access(appDataDir);
        ...
        await dialog.showErrorBox('LEAR Error', `Invalid project: The selected folder is not a valid LEAR project. Please select a folder containing a 'data/_appdata' directory.`);
        return null;
    }
}
            ...
            submenu: [
                {
                    ...
                    click: async () => {
                        try {
                            ...
                            await dialog.showErrorBox('LEAR Error', `New project failed: ${error.message}`);
                        }
                    }
                },
                {
                    ...
                    submenu: [
                        {
                            ...
                            click: async () => {
                                try {
                                    ...
                                    await dialog.showErrorBox('LEAR Error', `Open project failed: ${error.message}`);
                                }
                            }
                        },
                        { type: 'separator' },
...
async function restartApp(dataDir, win) {
    ...
    try {
        ...
        await dialog.showErrorBox('LEAR Error', `Failed to start server: ${error.message}`);
        return;
    }
    ...
    try {
        ...
        await dialog.showErrorBox('LEAR Error', `Failed to load UI: ${error.message}`);
    }
}
