...
export class ProjectService {
    ...
    onRefreshProject(loadFull = false): boolean {
        ...
        this.storage.getStorageProject(loadFull).subscribe(prj => {
            ...
            if (requestId === this.projectLoadRequestId) {
                console.error('LEAR onRefreshProject error', err);
            }
        });
        return true;
    }
    ...
    private load(loadFull = false) {
        ...
        this.storage.getStorageProject(loadFull).subscribe(prj => {
            ...
            if (requestId === this.projectLoadRequestId) {
                console.error('LEAR load error', err);
            }
        });
    }
    ...
    private notifySaveError(err: any) {
        console.error('LEAR notifySaveError error', err);
    ...
    private notifyServerError() {
        console.error('LEAR notifyServerError error');
        let msg = null;
    ...
    /**
     * Check to add or remove system Tags, example connection status to add in device LEAR server
     */
    checkSystemTags() {
